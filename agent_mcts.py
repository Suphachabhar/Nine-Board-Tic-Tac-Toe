...
import socket,sys,time,math,pickle,os
import numpy as np
...
class Board(object):
    ...
    def parse(self,string):
        ...
        if command == "win" or command == "loss":
            max_bytes = 2**31 - 1

            play_bytes_out = pickle.dumps(self.mtcs.num_plays)
            with open('play.dictionary', 'wb') as play_dictionary_file:
                for idx in range(0,len(play_bytes_out),max_bytes):
                    play_dictionary_file.write(play_bytes_out[idx:idx+max_bytes])
            win_bytes_out = pickle.dumps(self.mtcs.num_wins)
            with open('win.dictionary', 'wb') as win_dictionary_file:
                for idx in range(0,len(win_bytes_out),max_bytes):
                    win_dictionary_file.write(win_bytes_out[idx:idx+max_bytes])
...
class MCTS(object):
    def __init__(self):
        ...
        try :
            max_bytes = 2**31 - 1
            win_bytes_in = bytearray(0)
            win_input_size = os.path.getsize('win.dictionary')
            with open('win.dictionary', 'rb') as win_dictionary_file:
                for _ in range(0,win_input_size,max_bytes):
                    win_bytes_in += win_dictionary_file.read(max_bytes)
            self.num_wins = pickle.loads(win_bytes_in)

            play_bytes_in = bytearray(0)
            play_input_size = os.path.getsize('play.dictionary')
            with open('play.dictionary', 'rb') as play_dictionary_file:
                for _ in range(0,play_input_size,max_bytes):
                    play_bytes_in += play_dictionary_file.read(max_bytes)
            self.num_plays = pickle.loads(play_bytes_in)
    ...
    def run_simulation(self):
        ...
        while True:
            ...
            #     temp_boards = state.boards.copy()
            #     next_player = self.game_board.current_player(state)
            #     temp_boards[state.curr_board][move] = next_player
            #     if self.game_board.is_subboard_won(next_player,temp_boards[state.curr_board]):
            #         if player == 1:
            #             legal_moves = [move]
            #         else :
            #             if len(legal_moves) > 1:
